...
```{r}
model.reduced = lm(V8 ~ V9 + V10 +  V11 + V12 + V16, data=data2)
...
```{r}
model.reduced2 = lm(V8 ~  V9 + V11 + V12 + V15 + V16, data=data2)
...
```{r}
model.reduced2 = lm(V8 ~ V9 + V12 + V15 + V16, data=data2)
summary(model.reduced2)
anova(model.reduced2, model.full)
```
...
Trying to find highly correlated points

```{r}
round(cor(data[,-c(1, 2, 3, 8)]), dig=2)
```

```{r}
M = cor(data[,-c(1, 2, 3, 9, 16, 5)])
corrplot(M, tl.srt = 45)
```

Trying to find a grouping

```{r}
anova(lm(V8 ~ ., data2), lm(V8 ~ . -V12 - V13 + I(V12+V13), data2))
```

Backwards Selection to choose variables

```{r}
### Backward Selection Algorithm :) ###
data_to_use = data[,-c(1, 2, 3)]
...
V5 looks highly correlated with V16, and we can drop it
...
```{r}
anova(lm(V8 ~ V9 + V16 + I(V12 + V13), data_scaled), lm(V8~., data_scaled))
```

Hol up this shit looks gooooood

```{r}
summary(lm(V8 ~ V9 + V16 + I(V12 + V13), data_scaled))
```

```{r}
plot(lm(V8^-2 ~ V9 + V16 + I(V12 + V13),data_scaled))
```

```{r}
library(lmtest)
bptest(lm(V8^-2 ~ V9 + V16 + I(V12 + V13),data_scaled))
```

**Still not normal but idk how to fix :(**
...
```{r}
data2.no_bad_highlev = data2[-as.integer(rownames(data.highlev2)),]
model.full.no_bad_highlev = lm(V8 ~ . , data=data2.no_bad_highlev)
summary(model.full.no_bad_highlev)
```

```{r}
p_vals = summary(model.full.no_bad_highlev)$coef[, "Pr(>|t|)"]
names(p_vals[p_vals < .25])
```

```{r}
model.reduced.no_bad_highlev = lm(V8 ~ V5 + V9 +  V12 + V15 + V16, data=data2.no_bad_highlev)
summary(model.reduced.no_bad_highlev)
anova(model.reduced.no_bad_highlev, model.full.no_bad_highlev)
```

```{r}
plot(model.reduced.no_bad_highlev)
```
...
```{r}
data2.no_outliers = data2[-as.integer(names(data.outliers)),]

model.full.no_outliers = lm(V8~ ., data=data2.no_outliers)
summary(model.full.no_outliers)
```

```{r}
p_vals = summary(model.full.no_outliers)$coef[, "Pr(>|t|)"]
names(p_vals[p_vals < .25])
```

```{r}
model.reduced.no_outliers = lm(V8 ~ V5 + V9 + V10 + V12 + V15 + V16, data=data2.no_outliers)
summary(model.reduced.no_outliers)
anova(model.reduced.no_outliers, model.full.no_outliers)
```

```{r}
plot(model.reduced.no_outliers)
```
...
```{r}
data.no_inf = data2[-1, ]
model.full.no_inf = lm(V8 ~ ., data=data.no_inf)
summary(model.full.no_inf)
```

```{r}
model.reduced.no_inf = lm(V8 ~ V5 + V9 + V11 + V15 + V12 + V16, data.no_inf)
summary(model.reduced.no_inf)
anova(model.reduced.no_inf, model.full.no_inf)
```

```{r}
plot(model.reduced.no_inf)
```
...
```{r}
sqrt_transform = lm(sqrt(V8) ~ V9 + V11 + V12 + V15 + V16, data=data2)
...
```{r}
log_transform = lm(log(V8) ~ V9 + V11 + V12 + V15 + V16, data=data2)
...
```{r}
inv_transform = lm(V8^-1 ~ V9 + V11 + V12 + V15 + V16, data=data2)
...
```{r}
model.tr = lm(((V8^lambda-1)/lambda) ~ ., data=data2)
summary(model.tr)
```
...
Trying this w/ no outliers or bad high leverages

```{r}
data_no_bad = data2[-union(as.integer(rownames(data.highlev2)), as.integer(names(data.outliers))),]

bc = boxcox(data_no_bad$V8 ~ ., data=data_no_bad)
lambda = bc$x[which.max(bc$y)]
```

```{r}
model.tr.no_bad = lm((V8^lambda-1)/lambda ~ ., data=data_no_bad)
summary(model.tr)
```

```{r}
plot(model.tr.no_bad, col=2)
```

```{r}
bptest(model.tr.no_bad)
```

```{r}
hist(model.tr.no_bad$residuals, breaks=100)
```

```{r}
y = data_no_bad$V8
ks.test(1/(y^lambda-1)/lambda, "pnorm")
```
